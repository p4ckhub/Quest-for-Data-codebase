...
This phase is ordered so each task is independently testable, and so the **earliest deliverable is "compiles and plays via `npx electron dist/app/main.js` on Windows from source"** — a real distributable installer (electron-builder packaging) is deliberately pushed to the last, optional task.
...
| WP-7 | End-to-end verification: `lessons:validate`, full `vitest`, `e2e-drive` smoke, manual play session (`npx electron dist/app/main.js`) — the actual "plays on Windows" milestone | WP-4, WP-6 |
...
5. Manual play session on an actual Windows machine. The full from-clone sequence (verified 2026-07-12 — earlier drafts of this doc said `npx electron .`, which is wrong: root `package.json` `main` is a nonexistent `index.js`, and `ui/` is its own npm package whose deps root `npm ci` does NOT install):

   ```
   git clone -b Windows <repo> && cd Quest-for-Data-codebase
   npm ci                       # root deps (electron, ts-node, vitest…)
   cd ui && npm ci && cd ..     # UI deps — separate package, required for npm run build
   npm run toolchain:fetch      # one-time ~215 MB WinLibs download
   npm run build                # renderer → ui/dist/
   npx tsc                      # main process → dist/app/main.js (exits nonzero on ~100 pre-existing type errors; emit still happens — ignore)
   npx electron dist/app/main.js
   ```

   Then: load a save, play through a few lessons of different `kind` (`program` and `functions`), confirm a compile error renders its friendly hint (this is the concrete proof that the MinGW-diagnostics choice in WP-5 paid off), confirm a runtime crash lesson (e.g. one of the `EXIT_STATUS_ROWS` stack-overflow-teaching lessons) renders correctly.

**How to verify:** all of the above pass on a real Windows environment. This is the milestone this doc calls "compiles and plays on Windows from source" — treat it as the phase's actual finish line; WP-8 is explicitly a stretch goal beyond it.
