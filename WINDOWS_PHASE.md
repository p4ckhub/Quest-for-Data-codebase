...
4. **Bundling vs. fetching MinGW-w64 (WP-5)** — whether to commit a MinGW-w64 distribution's binaries into the repo (large, but simple and matches how `sandbox_run`'s Linux binary is apparently already handled — built manually and committed/copied into `toolchain/bin/`) versus writing a real fetch step in `scripts/fetch-toolchain.ts` that downloads and extracts an archive on first run (smaller repo, but adds a network dependency and download/extract/verify code that doesn't exist today for any profile). Left open — this is a repo-hygiene/size tradeoff decision, not a technical blocker either way.
