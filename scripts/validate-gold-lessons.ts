...
const { runLesson } = require('../runner/src/index.ts');
...
async function validateGoldLessons() {
  ...
  if (exitCode === 0) {
    ...
    try {
      ...
      console.log('runLesson result:', JSON.stringify(r1, null, 2));
      if (r1.success) {
        console.log('✓ Lesson 1 solution ran successfully');
        lessonsValidated++;
      } else {
        console.error('✗ Lesson 1 solution failed:');
        console.error('compileError:', r1.compileError || 'none');
        console.error('error:', r1.error || 'none');
        console.error('output:', r1.output || 'none');
        exitCode = 1;
      }
    } catch (e) {
  ...
  if (exitCode === 0) {
    ...
    try {
      ...
      if (r2.success) {
        ...
        console.error('compileError:', r2.compileError || 'none');
        console.error('error:', r2.error || 'none');
        console.error('output:', r2.output || 'none');
        exitCode = 1;
      }
    } catch (e) {
